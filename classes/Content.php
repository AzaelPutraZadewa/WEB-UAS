...
Class Content extends DBConnection {
	...
	public function teams(){
		...
		if(empty($id)){
			$sql ="INSERT INTO teams set $data";
		}else{
			$sql ="UPDATE teams set $data where id = {$id}";
		}
		$save = $this->conn->query($sql);
		$action = empty($id) ? "added":"updated";
		if($save){
			...
			$resp['status']='success';
			$resp['message']= " Teams Details successfully ".$action;
			$this->settings->set_flashdata('success',$resp['message']);
	...
	public function teams_delete(){
		extract($_POST);
		$fpath = $this->conn->query("SELECT file_path FROM teams where id = $id")->fetch_array()['file_path'];
		$qry = $this->conn->query("DELETE FROM teams where id = $id");
		if($qry){
			if(is_file(base_app.$fpath))
					unlink(base_app.$fpath);
			$resp['status']='success';
			$resp['message']= " Teams Details successfully deleted";
			$this->settings->set_flashdata('success',$resp['message']);
		}else{
			$resp['status']='success';
			$resp['error']= $this->conn->error;
			$resp['err_msg']= " Teams Details has failed to delete";
		}
		return json_encode($resp);
