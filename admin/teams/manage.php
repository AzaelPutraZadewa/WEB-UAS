...
<?php 
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT * from teams where id = '{$_GET['id']}' ");
...
<div class="col-lg-12">
	<div class="card card-outline card-primary">
		<div class="card-header">
			<h5 class="card-title"><?php echo isset($id) ? "Manage" : "Create" ?> teams</h5>
		</div>
		<div class="card-body">
			<form id="teams">
		...
		<div class="card-footer">
			<button class="btn btn-primary btn-sm" form="teams"><?php echo isset($_GET['id']) ? "Update": "Save" ?></button>
			<a class="btn btn-primary btn-sm" href="./?page=teams">Cancel</a>
		</div>
	</div>
</div>
...
<script>
	...
	$(document).ready(function(){
		...
		$('#teams').submit(function(e){
			...
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Content.php?f=teams",
				...
				success:function(resp){
					if(resp != undefined){
						if(resp.status == 'success'){
							location.href=_base_url_+"admin/?page=teams";
						}else{
							alert_toast("An error occured",'error')
							console.log(resp);
						}
